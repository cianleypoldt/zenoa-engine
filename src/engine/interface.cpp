...
uint32_t rbs::get_max_entity_count(system_context *  /*cntx*/) {
...
void rbs::add_circle_collider(system_context * cntx, unsigned int id, float radius, float density = 0.0005) {
    ...
    cntx->entity_manager.bodies.collider_shape[id].circle_col.radius = radius;
    cntx->entity_manager.bodies.mass[id]                             = 3.14f * radius * radius * (density);
    cntx->entity_manager.bodies.inv_mass[id]                         = 1 / cntx->entity_manager.bodies.mass[id];
    cntx->entity_manager.bodies.inertia[id]     = 0.5f * cntx->entity_manager.bodies.mass[id] * radius * radius;
...
void rbs::add_convex_collider(system_context *       cntx,
    ...
    auto & cc = em.bodies.collider_shape[id].convex_col;
    cc.begin  = cntx->vertex_pool.size();
    cntx->vertex_pool.insert(cntx->vertex_pool.end(), vertices.begin(), vertices.end());
    cntx->vertex_pool_worldspace.insert(cntx->vertex_pool_worldspace.end(), vertices.begin(), vertices.end());
...
[[nodiscard]] float rbs::get_circle_radius(system_context * cntx, uint32_t id) {
    ...
    return cntx->entity_manager.bodies.collider_shape[id].circle_col.radius;
}
...
[[nodiscard]] std::vector<glm::vec2> rbs::get_convex_verticies(system_context * cntx, uint32_t id) {
    ...
    return std::vector<glm::vec2>(
        cntx->vertex_pool.begin() + cntx->entity_manager.bodies.collider_shape[id].convex_col.begin,
        cntx->vertex_pool.begin() + cntx->entity_manager.bodies.collider_shape[id].convex_col.end);
}
