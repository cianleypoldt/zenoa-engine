...
void IntegrationSystem::update_convex_worldspace_verticies(system_context* system_cntx) {
    for (int index = 0; index < system_cntx->entity_manager.convex_colliders.count; index++) {
        ...
        for (uint32_t vertex_index = system_cntx->entity_manager.bodies.collider[this_id].convex.begin;
             vertex_index < system_cntx->entity_manager.bodies.collider[this_id].convex.end;
             vertex_index++) {
