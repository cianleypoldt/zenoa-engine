    ...
    for (uint32_t i = 0; i < cntx->entity_manager.circle_colliders.count; i++) {
        ...
        for (uint32_t j = 0; j < cntx->entity_manager.circle_colliders.count; j++) {
            ...
            if (circle_a != circle_b) {
                circle_circle_collision(cntx, circle_a, circle_b);
            }
        }
    }
    for (uint32_t i = 0; i < cntx->entity_manager.circle_colliders.count; i++) {
        ...
        for (uint32_t j = 0; j < cntx->entity_manager.convex_colliders.count; j++) {
            ...
            circle_convex_collision(cntx, circle, convex);
        }
    }
    for (uint32_t i = 0; i < cntx->entity_manager.convex_colliders.count; i++) {
        ...
        for (uint32_t j = 0; j < cntx->entity_manager.convex_colliders.count; j++) {
            ...
            if (convex_a != convex_b) {
                convex_convex_collision(cntx, convex_a, convex_b);
            }
        }
    }
}
...
void CollisionSystem::circle_circle_collision(system_context* cntx, uint32_t id_a, uint32_t id_b) {
    ...
    float total_inv_mass = bodies.inv_mass[id_a] + bodies.inv_mass[id_b];
    if (total_inv_mass > 0) {
        bodies.position[id_a] -= collision_normal_ab * overlap * (bodies.inv_mass[id_a] / total_inv_mass);
        bodies.position[id_b] += collision_normal_ab * overlap * (bodies.inv_mass[id_b] / total_inv_mass);
    }
    ...
    float denominator            = bodies.inv_mass[id_a] + bodies.inv_mass[id_b];
    float impulse                = numerator / denominator;

    bodies.velocity[id_a] -= collision_normal_ab * impulse * bodies.inv_mass[id_a];
    bodies.velocity[id_b] += collision_normal_ab * impulse * bodies.inv_mass[id_b];
    ...
    float friction_denominator = bodies.inv_mass[id_a] + bodies.inv_mass[id_b] +
                                 bodies.inv_inertia[id_a] * radius_a * radius_a +
                                 bodies.inv_inertia[id_b] * radius_b * radius_b;

    float friction_impulse = -rv_along_surface / friction_denominator;

    friction_impulse = glm::clamp(
        friction_impulse, -friction_coefficient * std::abs(impulse), friction_coefficient * std::abs(impulse));

    glm::vec2 friction_force = collision_tangent * friction_impulse;

    bodies.velocity[id_a] -= friction_force * bodies.inv_mass[id_a];
    bodies.velocity[id_b] += friction_force * bodies.inv_mass[id_b];
    ...
    bodies.angular_velocity[id_a] += torque_a * bodies.inv_inertia[id_a];
...
void CollisionSystem::circle_convex_collision(system_context* cntx, uint32_t id_circle, uint32_t id_convex) {
    ...
    if (glm::length(relative_pos) > circle_radius + convex.bounding_radius) {
        return;
    }
    ...
    for (uint i = convex.begin; i < convex.end; i++) {
        ...
        if (circle_to_p1.x * edge.y - circle_to_p1.y * edge.x > 0) {
            continue;
        }
        inside                = false;
    ...
    float total_inv_mass = bodies.inv_mass[id_circle] + bodies.inv_mass[id_convex];
    if (total_inv_mass > 0) {
        bodies.position[id_circle] -= collision_normal * overlap * (bodies.inv_mass[id_circle] / total_inv_mass);
        bodies.position[id_convex] += collision_normal * overlap * (bodies.inv_mass[id_convex] / total_inv_mass);
    }
    ...
    float denominator = bodies.inv_mass[id_circle] + bodies.inv_mass[id_convex] +
                        bodies.inv_inertia[id_convex] * r_convex_cross_n * r_convex_cross_n;

    float impulse = -(elasticity * rv_along_normal) / denominator;

    // Apply impulse to linear and angular velocities
    bodies.velocity[id_circle] -= collision_normal * impulse * bodies.inv_mass[id_circle];
    bodies.velocity[id_convex] += collision_normal * impulse * bodies.inv_mass[id_convex];
    bodies.angular_velocity[id_convex] += impulse * r_convex_cross_n * bodies.inv_inertia[id_convex];
    ...
    float friction_denominator = bodies.inv_mass[id_circle] + bodies.inv_mass[id_convex] +
                                 bodies.inv_inertia[id_circle] * r_circle_cross_t * r_circle_cross_t +
                                 bodies.inv_inertia[id_convex] * r_convex_cross_t * r_convex_cross_t;
    ...
    bodies.velocity[id_circle] -= tangent * friction_impulse * bodies.inv_mass[id_circle];
    bodies.velocity[id_convex] += tangent * friction_impulse * bodies.inv_mass[id_convex];
    bodies.angular_velocity[id_circle] -= friction_impulse * r_circle_cross_t * bodies.inv_inertia[id_circle];
...
void CollisionSystem::convex_convex_collision(system_context* cntx, uint32_t id_a, uint32_t id_b) {
    ...
    if (total_inv_mass > 0) {
        ...
        bodies.position[id_a] -= smallest_axis * min_overlap * (bodies.inv_mass[id_a] / total_inv_mass);
        bodies.position[id_b] += smallest_axis * min_overlap * (bodies.inv_mass[id_b] / total_inv_mass);
    }
    ...
    glm::vec2 rel_velocity = bodies.velocity[id_b] +
                             glm::vec2(-r_b.y * bodies.angular_velocity[id_b], r_b.x * bodies.angular_velocity[id_b]) -
                             bodies.velocity[id_a] -
                             glm::vec2(-r_a.y * bodies.angular_velocity[id_a], r_a.x * bodies.angular_velocity[id_a]);
    ...
    // Calculate impulse scalar
    float impulse_denominator = bodies.inv_mass[id_a] + bodies.inv_mass[id_b] +
                                bodies.inv_inertia[id_a] * r_a_cross_n * r_a_cross_n +
                                bodies.inv_inertia[id_b] * r_b_cross_n * r_b_cross_n;

    float impulse = -(elasticity * vel_along_normal) / impulse_denominator;

    // Apply impulse to linear and angular velocities
    bodies.velocity[id_a] -= smallest_axis * impulse * bodies.inv_mass[id_a];
    bodies.velocity[id_b] += smallest_axis * impulse * bodies.inv_mass[id_b];
    bodies.angular_velocity[id_a] -= impulse * r_a_cross_n * bodies.inv_inertia[id_a];
    bodies.angular_velocity[id_b] += impulse * r_b_cross_n * bodies.inv_inertia[id_b];
    ...
    float friction_denominator = bodies.inv_mass[id_a] + bodies.inv_mass[id_b] +
                                 bodies.inv_inertia[id_a] * r_a_cross_t * r_a_cross_t +
                                 bodies.inv_inertia[id_b] * r_b_cross_t * r_b_cross_t;
    ...
    // Apply friction impulse
    bodies.velocity[id_a] -= tangent * friction_impulse * bodies.inv_mass[id_a];
    bodies.velocity[id_b] += tangent * friction_impulse * bodies.inv_mass[id_b];
    bodies.angular_velocity[id_a] -= friction_impulse * r_a_cross_t * bodies.inv_inertia[id_a];
    bodies.angular_velocity[id_b] += friction_impulse * r_b_cross_t * bodies.inv_inertia[id_b];
}
